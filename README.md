...
```bash
# 追踪所有模块的执行
TtdToTrace.exe -f trace.run

# 追踪指定模块
TtdToTrace.exe -f trace.run -m target.dll

# 追踪多个模块
TtdToTrace.exe -f trace.run -m module1.dll,module2.exe

# 指定时间范围（格式为十六进制）
TtdToTrace.exe -f trace.run -s a:0 -e ff:0
```
